...
async function loadQuestData() {
  try {
    ...
    if (!res.ok) throw new Error(`Could not find mods.json at ${res.url}`);
    questState.modsData = await res.json();
  } catch(e) {
    console.error("Failed to load mods.json for quests", e);
    // Optional: notify user in the UI
    const questModList = document.getElementById('quest-mods-list');
    if(questModList) {
      questModList.innerHTML = `<div class="text-muted p-2" style="font-size:10px;">Error: Quest database (mods.json) not found.</div>`;
    }
  }
}
...
export function renderQuestEditor() {
  ...
  document.querySelectorAll('.btn-del-dep').forEach(el => {
