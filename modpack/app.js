...
function bindQuestUI() {
  ...
  qcContainer.addEventListener('mousedown', (e) => {
    ...
    } else if (e.target === qcContainer || e.target.id === 'quest-canvas-inner' || e.target.id === 'quest-nodes' || e.target.id === 'quest-edges') {
      questState.selectedQuestId = null;
      questState.linkSource = null;
      renderQuestCanvas();
      renderQuestEditor();
...
function renderQuestChapters() {
  ...
  questState.chapters.forEach((ch, idx) => {
    ...
    btn.style.cssText = `padding:4px 8px; font-size:11px; ${idx === questState.activeChapter ? 'background:var(--green); color:#000;' : ''}`;
    btn.textContent = `${ch.title} (${ch.quests.length})`;
    btn.addEventListener('click', () => {
...
function renderQuestCanvas() {
  ...
  const svg = document.getElementById('quest-edges');
  let edgesHtml = `
    <defs>
  ...
  chapter.quests.forEach(quest => {
    quest.dependencies.forEach(depId => {
      ...
      const y1 = (dep.y * 80) + 28;
      const x2 = (quest.x * 80) + 75;
      const y2 = (quest.y * 80) + 28;
...
function renderQuestEditor() {
  ...
  document.getElementById('qe-x').addEventListener('change', e => { quest.x = parseFloat(e.target.value)||0; saveQuestState(); renderQuestCanvas(); });
  document.getElementById('qe-y').addEventListener('change', e => { quest.y = parseFloat(e.target.value)||0; saveQuestState(); renderQuestCanvas(); });
