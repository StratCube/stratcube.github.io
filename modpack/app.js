...
function renderQuestEdges() {
  ...
  chapter.quests.forEach(quest => {
    quest.dependencies.forEach(depId => {
      ...
      // Node Box properties. Size is 150px wide. Height is ~70px based on content padding.
      const W = 150;
      const H = 70;
      ...
      if (distance < 10) return; // Prevent drawing if exactly overlapping
      
      const angle = Math.atan2(dy, dx);
      const cos = Math.cos(angle);
      const sin = Math.sin(angle);
      
      // Rectangular bounding box intersection to prevent line going inside the node
      // For Target Node (Leaves room for arrow head)
      ...
      // For Source Node (Line starts exactly at the border)
      ...
      // Avoid drawing weird inverted lines if nodes are dragged too close
