...
let state = {
  ...
  mcVersion: "",
  loader: "fabric",
  loaderVersion: "", // Added back for manual override
...
function loadState() {
  ...
  if (saved) {
    ...
    state.loaderVersion = parsed.loaderVersion || ""; // Load loader version
...
function bindUI() {
  ...
  document.getElementById('loader-version').addEventListener('input', e => { state.loaderVersion = e.target.value; saveState(); }); // Bind new input
...
function populateForm() {
  ...
  document.getElementById('loader-version').value = state.loaderVersion; // Populate new input
}
...
function updateWebLLMState() {
  ...
  if(isValid) {
    btn.disabled = false;
    btn.innerHTML = `Generate Quests<br><span class="text-xs text-muted">AI model may need to download</span>`;
  } else {
    btn.disabled = true;
  ...
  hits.forEach(hit => {
    ...
    if(!isAdded) {
      ...
      btn.addEventListener('click', async () => {
        ...
        state.mods.push({
          id, slug, title,
          category: defaultCat.charAt(0).toUpperCase() + cat.slice(1),
...
async function exportMrPack() {
  ...
  try {
    ...
    // !! CRITICAL: If no version is found, abort and tell user.
    if (!loaderVersion) {
    ...
    // 2. Generate modrinth.index.json
    const indexJson = {
