...
function bindUI() {
  ...
  // Export Buttons (Binds both the left panel and right panel buttons)
...
async function resolveDependencies() {
  ...
  let addedCount = 0;
  // Initialize queue with all current modrinth mods
  let queue = state.mods.filter(m => m.source === 'modrinth').map(m => m.slug);
  // Track processed projects by their ID to prevent duplicate requests/infinite loops
  ...
  try {
    while(queue.length > 0) {
      ...
      // Fetch version info for the current mod to inspect its dependencies
      ...
      for(let dep of version.dependencies) {
        ...
        if(dep.dependency_type === 'required' && dep.project_id) {
          if(!processed.has(dep.project_id)) {
            ...
            // Fetch project details for metadata (title, slug, categories)
            const projRes = await fetch(`https://api.modrinth.com/v2/project/${dep.project_id}`);
            if(!projRes.ok) continue;
            const proj = await projRes.json();
            
            // Fetch version to get the mrpack download file hash
            ...
            if(depVersions.length > 0) {
              ...
              addedCount++;
              queue.push(proj.slug); // Add nested dependency to queue
            }
          }
        }
      }
    ...
    saveState();
    searchMods(); // Refresh buttons in UI
  } catch (err) {
...
async function exportMrPack() {
  ...
  // Target all export buttons to disable them & change text
  ...
  try {
    ...
    // Dynamically fetch the correct loader version from Prism meta API
    let loaderVersion = "*";
    try {
      const prismUidMap = {
      ...
      if (uid) {
        ...
        if (metaRes.ok) {
          ...
          // Find versions of the loader that support the chosen Minecraft version
          const validVersions = metaData.versions.filter(v => 
            v.requires && v.requires.some(req => req.uid === 'net.minecraft' && req.equals === state.mcVersion)
          );
          if (validVersions.length > 0) {
            // Find recommended, otherwise default to the newest supported version
      ...
      console.warn("Failed to fetch loader version. Modrinth launcher will attempt to resolve automatically.", e);
    }

    // Map Modrinth spec loader keys
    const loaderKeyMap = {
    ...
    // Inject the loader into the dependencies list
    indexJson.dependencies[loaderKey] = loaderVersion;

    // 2. Process Mods
    state.mods.forEach(mod => {
      if (mod.custom) {
        // Find Blob in memory for custom uploaded mods
